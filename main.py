...
playerX = "X"
playerO = "O"
curr_player = random.choice([playerO,playerX])  # the starting player of the game is gone be random  
board = [[0, 0, 0], 
         [0, 0, 0], 
         [0, 0, 0]]
...
window.title("Tic Tac Toe")  #the title for the window
window.resizable(False, False) # you can not expand the window 

frame = tkinter.Frame(window)  # window->frame->label
label = tkinter.Label(frame, text=curr_player+"'s turn", font=("Consolas", 20), background=color_gray,
                      foreground="white")
label.grid(row=0, column=0, columnspan=3, sticky="we") #sticky="we"-> west 
...
window.mainloop() # a loop that keep the window active 